...
[[ -f .venv/bin/activate ]] || {
  echo "[ERRO] Execute primeiro: ./setup.sh --clean" >&2
  exit 1
}
...
if b"L202401" not in source:
    raise SystemExit("Lote original não encontrado no payload.")

Path("output/data/payload_tampered.txt").write_bytes(
    source.replace(b"L202401", b"L999999", 1)
)
print("[OK] Payload alterado criado sem nova assinatura.")
PY
...
if [[ "$status" -ne 2 ]]; then
  echo "[ERRO] O cenário alterado retornou código $status; era esperado 2." >&2
  exit 1
fi

echo
echo "[OK] Demonstração concluída."
echo "  Cenário íntegro: VÁLIDO"
echo "  Cenário alterado: INVÁLIDO"
echo "  Artefatos locais: output/"
